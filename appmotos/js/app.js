...
function renderDashboard() {
  ...
  if (configOficina) document.getElementById('oficina-nome').innerText = configOficina.VALOR;
...
function renderClientes() {
  ...
  container.innerHTML = state.clientes.map(c => `
    <div class="card">
      <h3>${c.NOME} (${c.ID_CLIENTE})</h3>
      <p>Tel/Whats: ${c.WHATSAPP}</p>
  ...
  container.innerHTML = state.motos.map(m => {
    ...
    return `
      <div class="card">
        ...
        <p>Cliente: ${cliente.NOME || 'N/A'} | KM: ${m.KM_ATUAL}</p>
...
function toggleSound() {
  ...
  document.getElementById('btn-sound').innerText = state.soundEnabled ? '🔊 SOM ON' : '🔇 SOM OFF';
}
